...
for ((page = 1; page <= pages; page++)); do
    curl -s -u $user:$connectionToken "$BaseAPI/$GetAPI?per_page=100&page=$page" | jq -r ''"${jq_args}"'' | while IFS=: read -r Name ID Archived; do
        if [[ $Name != ".github" && $Archived == "false" ]]; then
            ...
            curl -s -u $user:$connectionToken $BaseAPI/repos/$org/$Name/branches | jq -r '.[].name' | while read -r BrancheName; do
            ...
            ([ $(printf *.cloc) != '*.cloc' ] || [ -f '*.txt' ]) && for j in $(ls -la *.cloc | awk '{print $9}'); do
                CMD1=$(cat $j | grep SUM: | awk '{print $5}')
                BRTAB2["$CMD1"]=${j%?????}
                NBTAB1+="$CMD1 "
                cat $j >>$NameFile.txt
            done && /bin/rm *.cloc || echo ""
